...
getEstimate <- function(type, sample, binLength = NULL, left, right, smoothOn = TRUE) {
  ...
  switch(type,
         "stitch" = {
           stitch = stitchPDF(sample)
           estimateData$x = stitch$x
           estimateData$y = stitch$pdf
         },
         "pdfe" = {
            debugOn = FALSE
            if (finiteLeft && finiteRight) {
              ...
              if (finiteRight) {
                ...
                if (finiteLeft) {
                  ...
                  pdfe = estimatePDF(sample, debug = debugOn, smooth = smoothOn)
                }
              }
            }
